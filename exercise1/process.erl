...
bind(Binded) ->
  receive
    {task1, start, Max_Messages, Timeout} -> 
      ...
      receivingNbroadcasting(Binded, Max_Messages, SentMap, ReceivedMap)
  end.
...
% the process will stop broadcast messages.


receivingNbroadcasting(B, Max_Messages, SentMap, ReceivedMap) ->
  if Max_Messages == -1 ->
       Wait = infinity;
     true -> 
       Wait = 0
  ...
  receive
      {hello, PID, _M} ->
        ...
        receivingNbroadcasting(B, Max_Messages, SentMap, NewReceivedMap);
  ...
  after Wait ->
      ...
      receivingNbroadcasting(B, NewMax, NewSentMap, ReceivedMap)
  end.
